...
# Backwards-compatible wrappers used in pr_intervals.ipynb
def open_prs_per_day(
    ...
    *,
    ...
    return effective_open_prs_per_day(
        intervals,
        start_col=start_col,
        end_col="end",
        effective_end_col=end_effective_col,
        asof=asof,
    )


def queue_prs_per_day(df: pl.DataFrame) -> pl.DataFrame:
    return effective_queue_prs_per_day(df)


def with_queue_window_durations(
    df_qw: pl.DataFrame,
    *,
    now_ts: datetime | None = None,
    clamp_nonpositive: bool = True,
) -> pl.DataFrame:
    return effective_queue_window_durations(
        df_qw,
        asof=now_ts,
...
def compute_quantiles(df: pl.DataFrame, quantiles: Iterable[float]) -> pl.DataFrame:
    return snapshot_queue_age_quantiles(df, quantiles)
